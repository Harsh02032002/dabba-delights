...
async function apiRequest<T = any>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        (error as any).message || `API request failed: ${response.status}`,
      );
    }
    ...
    const duration = performance.now() - startTime;
    console.error(
      `[API Error] ${endpoint} - ${duration.toFixed(0)}ms - ${error.message}`,
    );
    throw error;
  }
}
...
export const productAPI = {
  ...
  switchMenuVersion: (version: number) =>
    apiRequest("/products/menu-version", {
      method: "PATCH",
  ...
  suggestOptimisation: () => apiRequest("/products/optimise-suggestions"),

  // P — HEALTH SCORE
  // GET /products/health-score
  menuHealthScore: () => apiRequest("/products/health-score"),
  ...
  getLowStockProducts: () => apiRequest("/products/inventory/low-stock"),
  ...
  syncInventory: (items: { id: string; stock: number }[]) =>
    apiRequest("/products/inventory/sync", {
      method: "POST",
      body: JSON.stringify(items),
    }),
  ...
  createFromTemplate: (templateId: string) =>
    apiRequest("/products/from-template", {
      method: "POST",
  ...
  bulkAction: (data: { ids: string[]; action: string; value?: any }) =>
    apiRequest("/products/bulk/action", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  startBackgroundBulkJob: (data: any) =>
    apiRequest("/products/bulk/background", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  getInvestorDashboard: () => apiRequest("/products/investor-dashboard"),
};

// PUBLIC APIs (no auth needed — used by footer, about us, static pages)
export const publicAPI = {
  getPlatformConfig: () =>
    apiRequest("/public/platform-config", { authToken: null }),
};
...
export const sellerAPI = {
  ...
  bulkUpdateOrderStatus: (ids: string[], status: string) =>
    apiRequest(`/seller/orders/bulk/status`, {
      method: "PATCH",
      body: JSON.stringify({ ids, status }),
    }),
  getProfile: () => apiRequest("/seller/profile"),
  updateProfile: (data: Record<string, unknown>) =>
    apiRequest("/seller/profile", {
      method: "PUT",
      body: JSON.stringify(data),
    }),
  ...
  getNotificationPreferences: () =>
    apiRequest("/seller/settings/notifications"),
  ...
  changePassword: (current: string, newPass: string) =>
    apiRequest("/seller/settings/password", {
      method: "POST",
...
export const adminAPI = {
  ...
  getAnalytics: (period: string) =>
    apiRequest(`/admin/analytics?period=${period}`),
  ...
  createCampaign: (data: Record<string, unknown>) =>
    apiRequest("/admin/marketing/campaigns", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  resolveDispute: (id: string, status: string, resolution: string) =>
    apiRequest(`/admin/disputes/${id}/resolve`, {
      method: "POST",
  ...
  bulkProcessSettlements: (ids: string[]) =>
    apiRequest("/admin/settlements/bulk-process", {
      method: "POST",
      body: JSON.stringify({ ids }),
    }),
  ...
  createWarehouse: (data: Record<string, unknown>) =>
    apiRequest("/admin/warehouses", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  updateWarehouse: (id: string, data: Record<string, unknown>) =>
    apiRequest(`/admin/warehouses/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
  ...
  mapSellerToWarehouse: (warehouseId: string, sellerId: string) =>
    apiRequest(`/admin/warehouses/${warehouseId}/map-seller`, {
      method: "POST",
      body: JSON.stringify({ sellerId }),
    }),
  unmapSellerFromWarehouse: (warehouseId: string, sellerId: string) =>
    apiRequest(`/admin/warehouses/${warehouseId}/unmap-seller/${sellerId}`, {
      method: "DELETE",
    }),
...
export const userAPI = {
  getSellers: async (params?: Record<string, unknown>) => {
    ...
    const sellers = Array.isArray(res)
      ? res
      : Array.isArray(res?.sellers)
        ? res.sellers
  ...
  getRestaurants: async (params?: Record<string, unknown>) => {
    ...
    const restaurants = sellers.filter(
      (seller) => seller.type === "restaurant",
    );
  ...
  addToCart: (data: Record<string, unknown>) =>
    apiRequest("/user/cart/add", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  updateQuantity: (itemId: string, quantity: number) =>
    apiRequest("/user/cart/update", {
      method: "PATCH",
  ...
  topupWallet: (amount: number) =>
    apiRequest("/user/wallet/topup", {
      method: "POST",
      body: JSON.stringify({ amount }),
    }),
  verifyPayment: (data: Record<string, unknown>) =>
    apiRequest("/user/wallet/verify", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  placeOrder: (data: Record<string, unknown>) =>
    apiRequest("/user/orders/place", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  getOrders: (status?: string) =>
    apiRequest(`/user/orders${status ? `?status=${status}` : ""}`),
  getOrderById: (id: string) => apiRequest(`/user/orders/${id}`),
  ...
  rateOrder: (id: string, rating: number, review?: string) =>
    apiRequest(`/user/orders/${id}/rate`, {
      method: "POST",
      body: JSON.stringify({ rating, review }),
    }),
  rateMenuItem: (menuItemId: string, rating: number) =>
    apiRequest(`/user/menu/${menuItemId}/rate`, {
      method: "POST",
      body: JSON.stringify({ rating }),
    }),
  rateSeller: (sellerId: string, rating: number) =>
    apiRequest(`/user/sellers/${sellerId}/rate`, {
      method: "POST",
      body: JSON.stringify({ rating }),
    }),
...
export const deliveryAPI = {
  ...
  // Order Management
  getActiveOrders: () => apiRequest<any>("/delivery/orders/active"),
  ...
  updateDeliveryStatus: (
    orderId: string,
    data: { status: string; location?: { lat: number; lng: number } },
  ) =>
  ...
  // Earnings & Wallet
  getEarnings: () => apiRequest<any>("/delivery/earnings"),

  getWalletTransactions: () => apiRequest<any>("/delivery/wallet/transactions"),
  ...
  // Profile Management
  getPartnerProfile: () => apiRequest<any>("/delivery/profile"),

  getOrderHistory: () => apiRequest<any>("/delivery/order-history"),
