...
export default function AdminSellers() {
  ...
  const filteredSellers = mockSellers.filter(seller => {
    ...
    const matchesTab = 
      activeTab === 'all' ||
      (activeTab === 'verified' && seller.kycStatus === 'verified') ||
      (activeTab === 'pending' && seller.kycStatus === 'pending') ||
      (activeTab === 'home_chef' && seller.type === 'home_chef') ||
      (activeTab === 'restaurant' && seller.type === 'restaurant');
    ...
    <AdminLayout title="Seller Management" subtitle="Manage and approve sellers on the platform">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-card rounded-xl p-4 border border-border">
          ...
          <p className="text-2xl font-bold text-foreground">{mockSellers.length}</p>
        </div>
        <div className="bg-success/10 rounded-xl p-4">
          <p className="text-sm text-success">Verified</p>
          <p className="text-2xl font-bold text-success">
            {mockSellers.filter(s => s.kycStatus === 'verified').length}
          </p>
        </div>
        <div className="bg-warning/10 rounded-xl p-4">
          <p className="text-sm text-warning">Pending KYC</p>
          <p className="text-2xl font-bold text-warning">
            {mockSellers.filter(s => s.kycStatus === 'pending').length}
          </p>
        </div>
        <div className="bg-info/10 rounded-xl p-4">
          <p className="text-sm text-info">Active Today</p>
          <p className="text-2xl font-bold text-info">
            {mockSellers.filter(s => s.isActive).length}
          </p>
        </div>
      </div>
