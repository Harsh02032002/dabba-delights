...
export default function UserWallet() {
  ...
  const topupMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      // In real app, open Razorpay checkout here with data.orderId
      toast({ title: 'Top-up initiated', description: 'Complete payment to add funds.' });
    ...
    <UserLayout>
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-2xl font-display font-bold text-foreground">My Wallet</h1>
        </div>
          ...
          <>
            ...
            <Card className="mb-8 overflow-hidden">
              <div className="gradient-primary p-8 text-center">
                <Wallet size={40} className="mx-auto text-primary-foreground mb-3" />
                <p className="text-primary-foreground/80 text-sm mb-1">Available Balance</p>
                <p className="text-4xl font-bold text-primary-foreground">₹{wallet?.balance?.toLocaleString() || '0'}</p>
              </div>
            </Card>

            {/* Top Up */}
            <Card className="mb-8">
              <CardHeader><CardTitle className="font-display text-lg">Add Money</CardTitle></CardHeader>
              <CardContent>
                <div className="flex flex-wrap gap-2 mb-4">
                  {quickAmounts.map(amt => (
                    <Button key={amt} variant={topupAmount === String(amt) ? 'default' : 'outline'} size="sm" onClick={() => setTopupAmount(String(amt))}>
                      ₹{amt}
                    </Button>
                  ))}
                </div>
                <div className="flex gap-3">
                  ...
                  >
                    <Plus size={18} /> {topupMutation.isPending ? 'Processing...' : 'Add Money'}
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                    ...
                    <div key={txn._id} className="flex items-center justify-between p-3 rounded-xl bg-secondary/50">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${txn.type === 'credit' ? 'bg-success/10' : 'bg-destructive/10'}`}>
                          {txn.type === 'credit' ? <ArrowDownRight size={20} className="text-success" /> : <ArrowUpRight size={20} className="text-destructive" />}
                        </div>
                        <div>
                  ...
                  {history.length === 0 && <p className="text-muted-foreground text-center py-8">No transactions yet</p>}
                </div>
              </CardContent>
            </Card>
          </>
